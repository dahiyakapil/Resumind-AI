...
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Sparkles, UploadCloud, FileText, Download, RefreshCcw, Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";
import type { AnalysisResponse } from "@/types/resumeAnalysis";
import { toast } from "sonner";
...
export const RewriteBulletPointsPage: React.FC = () => {
  ...
  const [reportId, setReportId] = useState<string>(""); // ✅ now used
    ...
    <div className="min-h-screen px-6 py-10 text-foreground">
      ...
      {loading && (
        <div className="flex items-center gap-2 mb-4 text-sm text-blue-500">
          <Loader2 className="animate-spin w-4 h-4" /> Analyzing your resume...
        </div>
      )}
